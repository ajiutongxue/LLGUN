    ...
    @HostListener('mousemove', ['$event']) onmousemove(e) {
        if (this.isMouseDown) {
            ...
            if (e.target === this.tmpCvs) {
                ...
                if (this.qcMode) {
                    if (this.qcModeType === 'eraser') {
                        // if (this.ctx.isPointInPath(this.windowToCanvas(e.clientX, e.clientY).x, this.windowToCanvas(e.clientX, e.clientY).y)) {
                        if (this.ctx.isPointInPath(e.clientX, e.clientY)) {
                            this.eraserPointInPath = true;
                        }
                    }
                    console.log('>>>>>>>><<<<<<<< ' + this.eraserPointInPath);
                    this.draw(e);
                }
                if (this.zoomMode) {
                    this.isZoomOrMove = true;
    ...
    finishDraw = function () {
        if (this.isMouseDown) {
            ...
            if (this.qcModeType === 'pencil' || this.qcModeType === 'eraser') {
                if (this.qcModeType === 'eraser') {
                    console.log('current type is "eraser"!');
                    if (!this.eraserPointInPath) {
                        console.log('eraser not point in path ..');
