...
@Component({
    ...
    styleUrls: ['./my-player.component.less', './qc-msg-item.less']
})
export class MyPlayerComponent implements OnInit {
    ...
    vdo;
    pic = new Image();
    // picPlayDuration = 5000; // 图片播放时间；5s
    picCvs;
    picCtx;
    ...
    picReady = function () {
        ...
        this.META.owidth = this.pic.naturalWidth;
        this.META.oheight = this.pic.naturalWidth;
        this.META.duration = this.picPlayDuration;
    ...
    vdoReady = function () {
        // console.log('虽然是img， 但是还是loaded了');
        // todo: 从META里面拿出去，currenttime  currentframe
        if (this.vdoList[this.currentVdoIndex].type === 'img') {
            ...
            this.setCvsPos(this.picCtx, this.vdo);
            this.picCvs.style.zIndex = 8;
        } else {
