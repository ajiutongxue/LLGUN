...
export class PlayerLayerComponent implements OnInit {
    ...
    preload() {
        ...
        loader.addCompletionListener(() => {
            ...
            this.totalDuration = this.version_lists[0].data.reduce(
                (prev, cur) => {
            ...
            this.$scrollbar.on('mousedown', event => {
                ...
                $(document).on('mousemove.forSb', e => {
                    ...
                    if (scrollbarOffsetLeft + moveLength < 0) {
                        ...
                        // if (scrollbarOffsetLeft + moveLength > $('.scrollbar-box').width() - this.$scrollbar.outerWidth()) {
                        // if (scrollbarOffsetLeft + moveLength > this.verCvs.width - this.$scrollbar.outerWidth()) {
                        newLeft =
                            this.scrollbarBoxContentWidth -
                            this.$scrollbar.width()
                        // newLeft = this.verCvs.width - this.$scrollbar.outerWidth();
                    } else {
                        newLeft = scrollbarOffsetLeft + moveLength
                    }

                    // let newLeft = (scrollbarOffsetLeft + moveLength) < 0 ? 0 : (scrollbarOffsetLeft + moveLength);
                    // this.$scrollbar.css('left', (scrollbarOffsetLeft + moveLength) < 0 ? 0 : (scrollbarOffsetLeft + moveLength));
    ...
    preloadIcon() {
        const loader = new PxLoader()
        const img = loader.addImage('../assets/status-icon/ban.png')
        loader.addCompletionListener(() => {
            this.statusIcons = img
        })
        loader.start()
    ...
    getOffsetLeft() {
        return (
            this.getScrollbarOffsetLeft() / this.scrollbarBoxContentWidth * this.getLatestTotalCvsWidth()
        )
    }
    ...
    drawShadowRectsTheRow(rowData, rowIndex, c) {
        // if (!this.controlList[rowIndex].isShow) {
        //     c.clearRect(0, rowIndex * this.rectHeight, this.verCvs.width, this.rectHeight);
        //     return;
        // }
        rowData.forEach(r => {
            // 不要背景了
                ...
                if (r.picObj === null) {
                    ...
                    loader.addCompletionListener(() => {
                        ...
                        c.drawImage(
                            ...
                            0,
                            r.pic.ow,
                            r.pic.oh, // r.x + 1, r.y + 1, r.pic.w - 2, r.h - 2
                            r.x + 1,
                            r.y + this.WORDSHEIGHT + 1,
                            r.pic.w - 2,
                            r.pic.h - 2
                        )
                        // this.drawShadowWords(r)
    ...
    drawShadowWords(r) {
        ...
        // 16 是 icon 的宽度
        // const wordsWidth =
        //     c.measureText(wordsList[0]).width >
        //     c.measureText(wordsList[1]).width + 16
        //         ? c.measureText(wordsList[0]).width
        //         : c.measureText(wordsList[1]).width + 16
        ...
        wordsList.forEach((words, index) => {
            c.fillText(
                ...
                r.x + 16,
                index === 0 ? r.y : r.y + (this.rectHeight - this.WORDSHEIGHT)
                // r.y + r.h / 2 - this.WORDSHEIGHT / 2 + this.WORDSHEIGHT * index
            )
            // if (index === 1) {
            // c.save()
            ...
            c.drawImage(
                ...
                r.x + 2,
                index === 0 ? r.y + 3 : r.y + 3 + (this.rectHeight - this.WORDSHEIGHT),
                10,
                10
            )
            // c.restore()
    ...
    clickHandle(e) {
        ...
        /*const x = e.clientX;
         const y = e.clientY;
         // cvs Boundary
         const $cvs = $(this.verCvs);
         const $cvsParent = $cvs.parent();
         const l = $cvsParent.offset().left;
         const t = $cvsParent.offset().top;
         const ml = l + $cvsParent.width();
         const mt = t + $cvsParent.height();
         // console.log(`x: ${x}, y: ${y}`);
         if (x < l || x > ml || y < t || y > mt) { return; }

         const xInCvs = x - $cvs.offset().left;
         const yInCvs = y - $cvs.offset().top;
         let beforeActive;
         let afterActive;

         // set isActive = false
         this.rects.find((row) => {
         beforeActive = row.find((rect) => {
         return rect.isActive;
         });
         return beforeActive;
         });
         if (beforeActive) { beforeActive.isActive = false; }

         // set clicked rect isActive = true
         this.rects.find((row) => {
         afterActive = row.find((rect) => {
         return xInCvs > rect.x && xInCvs < (rect.x + rect.w) && yInCvs > rect.y && yInCvs < (rect.y + rect.h);
         });
         return afterActive;
         });
         if (afterActive) { afterActive.isActive = true; }
    ...
    whichClicked(e) {
        ...
        const rowIndex = Math.floor(
            (e.pageY - $(fc).offset().top) / this.rectHeight
        )
    ...
    drawActiveRect(indexs) {
        ...
        if (this.controlList[indexs['i']].isShowPic) {
            ...
            clipboard.width = r.pic.w
            clipboard.height = r.pic.h - 1
            clipboardCtx = clipboard.getContext('2d')
            clipboardCtx.drawImage(
                fcs,
                toX + 1, r.y + this.WORDSHEIGHT + 1, clipboard.width, clipboard.height,
        ...
        if (this.controlList[indexs['i']].isShowPic) {
            ...
            //     clipboard,
            //     0, 0, clipboard.width, clipboard.height,
            //     toX + 1, r.y + 18 + 1, clipboard.width, clipboard.height
            // );
        }
        c.save()
        c.strokeStyle = '#43C8FF'
