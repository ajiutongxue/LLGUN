...
export class MyPlayerComponent implements OnInit {
    ...
    vdoPlay = function () {
        ...
        this.playHandler = setInterval(() => {
            // this.clearCvs(this.ctx);
            // console.log('drawHistory?????????? ' + this.drawHistory.length);
            // console.log('drawHistoryCell?????????? ' + this.drawHistoryCell.length);
            // console.log('drawHistoryBackup?????????? ' + this.drawHistoryBackup.length);
    ...
    @HostListener('mousedown', ['$event']) onmousedown(e) {
        if (!this.watchMode && e.target.tagName.toLowerCase() === 'video' || e.target.tagName.toLowerCase() === 'canvas') {
